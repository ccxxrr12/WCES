...
/// (`network_reachable`, `circuit_breaker_open`, `cooldowns`,
/// `analysis_cache`) lives only in process memory. On restart
...
pub struct DegradationManager {
    ...
    pub(crate) analysis_cache: VecDeque<(u32, AnalysisResult, Instant)>,
}
...
impl DegradationManager {
    ...
    pub fn on_analysis_complete(&mut self, patient_id: u32, result: AnalysisResult) {
        ...
        // L10: VecDeque::pop_front() is O(1); the previous Vec::remove(0) was O(n).
        if self.analysis_cache.len() >= self.config.max_cache_size && !self.analysis_cache.is_empty() {
            self.analysis_cache.pop_front();
        }
        self.analysis_cache.push_back((patient_id, result, Instant::now()));
    }
