...
> 版本：v0.4 | 日期：2026-05-17
> 用途：供前端/AI开发新UI时参考，描述WebSocket推送的全部数据格式 + LLM HTTP API
...
| LLM 流式 | `ws://{host}:8765/ws` (发 `llm_analyze_request` 消息) |
...
| `sleep_apnea` | AHIUpdate(102) | info | 每小时呼吸暂停指数更新 |
...
| `seizure` | SeizureTonic(141) | critical | 强直期（高能量+低方差） |
| `seizure` | SeizureClonic(142) | critical | 阵挛期（3-8Hz节律） |
| `seizure` | PostIctal(143) | warning | 发作后恢复期 |
| `intrusion` | IntrusionAlert(200) | critical | 布防后检测到CSI振幅突变>3× |
| `intrusion` | IntrusionArmed(202) | info | 系统已布防（5秒无人静默） |
| `occupancy` | ZoneOccupied(300) | info | 子载波分区检测到人体占据 |
| `occupancy` | ZoneCount(301) | info | 当前占用的子载波分区数 |
| `occupancy` | ZoneEntered/ZoneLeft(302) | info | 分区占用状态变化 |
| `mincut` | PersonAssigned(720) | info | 多人MinCut特征分配 |
| `mincut` | PersonIDSwap(721) | warning | 人员ID交换检测 |
| `mincut` | MatchConfidence(722) | info | 多人匹配置信度聚合 |
| `weapon_detect` | MetalAnomaly(220) | warning | 幅度/相位比>4×（金属反射） |
| `weapon_detect` | WeaponAlert(221) | critical | 幅度/相位比>8×（疑似武器） |
| `weapon_detect` | CalibrationNeeded(222) | warning | 基线漂移>3×需重校准 |
| `sparse_recovery` | DropoutRate(717) | warning/info | 子载波丢失率 |
| `sparse_recovery` | RecoveryComplete(715) | info | ISTA迭代成功恢复缺失子载波 |
| `sparse_recovery` | RecoveryError(716) | warning | 丢失子载波>4个，恢复失败 |
| `gait` | StepCadence(130) | info | 步频检测（步/分钟） |
| `gait` | GaitAsymmetry(131) | warning | 左右步态不对称>15% |
| `gait` | FallRiskScore(132) | critical/warning/info | 跌倒风险评分（0-100） |
| `gait` | ShufflingDetected(133) | warning | 曳步检测（高步频+低变异） |
| `gait` | Festination(134) | critical | 慌张步态（步间逐渐缩短） |
| `loitering` | LoiteringStart(240) | warning | 静止>5分钟进入徘徊状态 |
| `loitering` | LoiteringOngoing(241) | warning | 徘徊持续中（每30秒提醒） |
| `loitering` | LoiteringEnd(242) | info | 徘徊结束（人员离开/恢复活跃） |
| `vibration` | SeismicDetected(540) | critical | 宽带相位RMS>0.15rad |
| `vibration` | MechanicalResonance(541) | warning | 窄带共振比>3× |
| `vibration` | StructuralDrift(542) | critical | 单调相位漂移>0.0005rad/帧 |
| `vibration` | VibrationSpectrum(543) | info | 振动频谱报告（每5秒） |
| `meta_adapt` | AdaptationScore(741) | info | 当前爬山优化最佳得分 |
| `meta_adapt` | RollbackTriggered(742) | info | 连续3次下降触发安全回滚 |
| `meta_adapt` | MetaLevel(743) | info | 当前迭代轮次 |
| `meta_adapt` | AdaptationParams(740) | info | 阈值调整参数报告 |
| `ltl` | LTLViolation(795) | critical/warning | 时序逻辑安全规则违反 |
| `ltl` | LTLSatisfaction(796) | info | 时序逻辑规则满足 |
| `ltl` | Counterexample(797) | info | 反例帧记录 |

> **模块编号 (1-19)：** 1.vital_trend 2.attractor 3.coherence(内部门控) 4.resp_distress 5.confined_space 6.panic_motion 7.sleep_apnea 8.cardiac 9.seizure 10.intrusion 11.occupancy 12.mincut 13.weapon_detect 14.sparse_recovery 15.gait 16.loitering 17.vibration 18.meta_adapt 19.ltl
...
## 10. LLM 流式分析 — StreamToken

当客户端通过 WebSocket 发送 `{"type":"llm_analyze_request","patient_id":"..."}` 时，
服务端返回 `StreamToken` 流（逐 token 推送）：

```json
{
  "survivor_id": "SURV-0042",
  "token_index": 0,
  "text": "伤员呼吸急促",
  "is_complete": false
}
...
| 字段 | 类型 | 说明 |
|------|------|------|
| `survivor_id` | `string` | 关联伤员 ID |
| `token_index` | `integer` | Token 序号，`u32::MAX` 表示流结束 |
| `text` | `string` | 当前 token 文本 |
| `is_complete` | `boolean` | 是否已完成（最后一个有效 token） |

> 客户端需累积 `text` 字段拼接完整分析报告。无 LLM 模型时自动降级为 fallback 模板分析。
> HTTP 端点 `POST /api/v1/llm/analyze` 通过 WebSocket upgreade 提供相同功能。
...
| `GET /api/v1/patients` | 列出所有注册患者 | `[{"patient_id":"...",...}]` |
| `POST /api/v1/patients` | 注册/更新患者 | `{"patient_id":"...","age":68,...}` → 200 OK |
| `POST /api/v1/llm/analyze` | 触发 LLM 流式分析 | WebSocket 升级，返回 `StreamToken` 流 |
| `GET /api/v1/llm/status` | LLM 引擎状态 | `{"patients_registered":N,"llm_loaded":bool,...}` |
