...
在野战方舱和灾后临时医院中，批量伤员到达后医护力量严重不足，传统接触式监护无法同时对数十名伤员进行连续监测，隐匿性恶化是这类场景中可预防死亡的首要原因。

本作品用WiFi信号解决这一问题。三个ESP32-C5节点构建WiFi 6感知网络，穿墙提取伤员呼吸、心率和体动。数据经UDP汇聚至瑞萨RZ/G2L主控，由纯Rust信号管线实时处理，再经START标准分诊协议将伤员自动分为红（立即救治）、黄（延迟）、绿（轻伤）、黑（死亡）、灰（数据不足）五级。全程无接触、零穿戴、不依赖云端。

核心技术贡献：（1）在ESP32-C5上实现PSRAM burst ring高帧率CSI采集，解决单射频半双工下的TX阻塞问题；（2）纯Rust管线对标MIT Vital-Radio/EQ-Radio学术标准，自适应采样率消除BPM系统误差；（3）SVD空房间校准+ISTA稀疏CIR+混合质心定位，三层校验实现3m以内的目标精度；（4）8维CSI生物特征嵌入实现伤员持续追踪与Re-ID；（5）Coordinator端云协同Agent，离线可用、云端增强、故障降级。

系统Rust编译0错误，12条端到端数据流全部接通，支持无硬件模拟演示。# 第一部分  作品概述
...
**自然灾害临时医院**。地震后临时医疗点大量伤员在无监护下转运，隐匿性恶化难以识别。德国Fraunhofer IIS指出单次10人以上伤亡事件中传统分诊仅能提供一次性生命体征记录。
...
**（1）WiFi 6 CSI感知**。ESP32-C5 802.11ax为20MHz-only，设双模协议（11AX+11N fallback），AP支持11ax时协商HE20 242子载波，否则回退HT40 114。C5单射频半双工开混杂后TX阻塞——实现PSRAM burst ring（256槽，8MB Quad SPI PSRAM），CSI回调写环形缓冲，每100ms定时flush，TX占用<2%，帧率10-50Hz。

**（2）Rust信号处理管线**。9 crates约9.2万行，两阶段写锁将锁持有时间压至微秒级。自适应采样率（帧间间隔EMA实时测量），滤波器系数、BPM窗口、运动阈值全部跟随。呼吸用IIR Butterworth带通+零交叉计数，心率用相位差分+自相关，对标Vital-Radio/EQ-Radio。解除子载波限制，242子载波全量参与生命体征计算。

**（3）多模态定位**。主定位用子载波方差邻近度（60%）+相位多普勒邻近度（40%）混合加权质心。辅以SVD空房间校准（12,000帧协方差分解→正交补投影提取人体扰动）和ISTA稀疏CIR（频域→时域→ToF测距）。6维Kalman融合所有观测。

**（4）START分诊与Re-ID**。分诊引擎直接实现START五级判定。8维CSI生物特征嵌入（呼吸/心率/体动/信号质量/RSSI+三项置信度），余弦匹配（0.75阈值），5分钟lost_pool缓冲。泄漏桶恶化检测实现从单次分诊到连续监护。

**（5）端云协同Agent**。边缘端本地管线保障离线可用；分诊恶化时异步触发云端LLM（限流+超时）；熔断器防级联故障；不可用时降级至本地模板。
...
1. **PSRAM burst ring**：在单射频半双工MCU上通过PSRAM环形缓冲+定时flush实现混杂模式高帧率CSI采集。

2. **242子载波全量利用**。解除HardwareNormalizer的56子载波归一化和VitalsBridge的子载波clamp，242子载波全量参与生命体征计算。

3. **物理场+SVD+稀疏恢复三层定位**。不依赖RSSI三角定位，从电磁场原理出发构建SVD校准+ISTA CIR估计+混合质心三层校验。

4. **8维CSI生物特征Re-ID**。用生命体征特征向量实现伤员持续追踪，隐私保护和环境鲁棒性优于摄像头方案。

5. **Coordinator端云协同**。边缘端离线可用，云端仅在恶化时触发增强分析，熔断器+模板降级确保不中断。## 设计流程

项目历时约两个月，遵循"人做设计决策，AI执行落地"的开发范式。

**硬件选型**：选定瑞萨RZ/G2L（双核A55）+ ESP32-C5（WiFi 6，242子载波），相比S3的114子载波翻倍。软件栈为Rust（服务端）+ C（固件）+ 原生JavaScript（前端）。

**算法迭代**：FFT不够精准，改用IIR带通+零交叉+自相关。RSSI三角定位在室内多径下误差太大，转而设计子载波方差-相位多普勒混合质心方案。

**工程约束**：C5单射频半双工下混杂模式TX阻塞——通过PSRAM burst ring解决。8MB Quad SPI PSRAM之前未启用——通过CONFIG_SPIRAM配置开启。七轮AI驱动代码审查，0编译错误，12条端到端数据流全部接通。# 第二部分  系统组成及功能说明
